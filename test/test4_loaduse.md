...
- **延迟时间**：1048576 × 3 × 40ns ≈ 126ms，闪烁频率约 4Hz
...
| 观察项 | 预期 |
|--------|------|
| LED[7:0] | 1/3/5/7 号灯亮（0xAA = 10101010），约 4Hz 闪烁 |
| LED[15:8] | 始终熄灭 |
...
在硬件上下载固件后，观察到 LED 1/3/5/7 以约 4Hz 频率闪烁（0xAA = 10101010 模式），LED 8~15 始终熄灭。这表明 Load-Use 冒险检测与停顿机制工作正常：lw 指令加载的数据 0xAA 经过 1 周期 stall 后通过 MEM/WB 前递正确送达后续 addi 指令，最终输出到 LED 的值为 0xAA。

| 观察项 | 实际结果 |
|--------|----------|
| LED[7:0] | 1/3/5/7 号灯亮（0xAA 模式），约 4Hz 闪烁 |
| LED[15:8] | 始终熄灭 |
