# =============================================================================
# build_forward.tcl - Build CPU Pipeline Forwarding Test Firmware
# =============================================================================
# This script does EVERYTHING in one step:
#   1. Copies the forwarding firmware to all locations
...
#   source C:/rvp_proj/synth/vivado/build_forward.tcl
#
# Usage in batch mode (from command line):
#   vivado -mode batch -source C:/rvp_proj/synth/vivado/build_forward.tcl
#
...
set fw_src      "C:/rvp_proj/synth/vivado/firmware_forward.hex"
...
puts "============================================================================="
puts " Pipeline Forwarding Test - Full Rebuild"
puts "============================================================================="
...
puts "   - LED 11 (0x0800) blinks at ~15Hz"
puts "   - 7-seg display shows delay loop PC"
puts "============================================================================="

# --- Step 1: Verify the forwarding firmware exists ---
if {![file exists $fw_src]} {
    set root_fw_src "C:/rvp_proj/firmware_forward.hex"
    if {[file exists $root_fw_src]} {
        puts "Copying firmware from root to synth dir..."
        file copy -force $root_fw_src $fw_src
        puts "\[OK\] Copied: $root_fw_src -> $fw_src"
    } else {
        puts "ERROR: Forwarding firmware not found: $fw_src"
        puts "Please generate it first at: C:/rvp_proj/firmware_forward.hex"
        return
    }
}

# --- Step 2: Copy firmware_forward.hex to ALL firmware.hex locations ---
puts ""
puts "--- Copying forwarding firmware to all locations ---"
...
if {[llength $bitfile] > 0} {
    ...
    puts ""
    puts "============================================================================="
    puts " SUCCESS! Pipeline Forwarding Test firmware built."
    puts "============================================================================="
