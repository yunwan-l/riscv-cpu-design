...
module tb_quick;
    ...
    assign dbus_r=0; initial begin clk=0; forever #5 clk=~clk; end
    string hp; initial begin #1;
        hp="D:/Course resources for the second semester of sophomore year/riscv-cpu-design/sw/tests/cache_big.hex";
    ...
    always@(posedge clk) if(sc>200||cyc>80000) begin
        $display("cache_big: hits=%0d miss=%0d rate=%.2f%%",ih,im,100.0*ih/(ih+im+1)); $finish; end
endmodule
